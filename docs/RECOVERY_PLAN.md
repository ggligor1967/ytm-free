...
3. **db tests — NEW sub-blocker (2026-07-06, 3rd session).** Now that the toolchain compiles, `cargo test` reports `20 passed; 8 failed` of 28. All 8 failures are in `db::tests` (migrations + CRUD); the other 20 (ollama/client, ollama/prompts, semantic, spotify_import) pass. **Root cause: pre-existing bug in `run_migrations` (`src-tauri/src/db.rs:27-73`)** — the baseline migration is version 0, but `last_version = COALESCE(MAX(version),0)` defaults to 0 on a fresh DB, so the guard `if migration.version > last_version` (`0 > 0`) is false and the initial schema migration never runs (`tracks`/`playlists`/etc. never created, no `schema_migrations` row). Fix (deferred — no source changes in the recovery session): renumber the initial migration to version 1, or change the guard/baseline so version 0 applies on an empty DB. This is a code fix, not a toolchain fix — it belongs to normal development, not Step 1.

Net: Step 1 is no longer blocked by the linker or by disk space. The Rust code **compiles** and tests **run**. The remaining `cargo test` failure is a real code bug (db migrations), not an environment problem — i.e. Rust Gate B is now **PASS-COMPILE-BUT-TEST-FAIL**, not BLOCKED-ENV.
