...
- The project is **feature-complete on paper but never validated end-to-end**, and the Rust half of the toolchain is broken on this machine as of 2026-07-06. Current truth: `PROJECT_STATE.md`.
...
3. **Rust changes are currently unverifiable** until the toolchain is repaired (`docs/RECOVERY_PLAN.md`). If you cannot run `cargo test`, say so in your report instead of assuming Rust code compiles.
...
## Commands that are BROKEN until the toolchain is fixed (verified 2026-07-06)

```
cargo test            # LNK1181 dbghelp.lib — see docs/RECOVERY_PLAN.md
npm run tauri dev
npm run tauri build
