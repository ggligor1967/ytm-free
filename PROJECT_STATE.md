...
| **Step 2B minimal runtime smoke currently FAILS in the stream path (verified 2026-07-06, 6th session)** — startup is still good, but the first read-only stream probe through the app's local server returns 500 for sampled search results. | `curl.exe -i http://localhost:3456/stream/sF80I-TQiW0` and `curl.exe -i http://localhost:3456/stream/YXZH-eBtmqQ` → `HTTP/1.1 500 Internal Server Error` with `Failed to get audio: Failed to execute yt-dlp: ERROR: [youtube] ... Requested format is not available`. Runtime log: `src-tauri` dev session logged `Failed to get audio URL for ...` from `ytm_free_lib::server`; source path is `src-tauri/src/server.rs` calling `ytdlp::get_audio_url` in `src-tauri/src/ytdlp.rs`. |
...
- Full end-to-end app flow (search → stream → download → playlist → restart → data persisted) — ROADMAP_STATUS.md item 1, "NEFACUT". Step 2A verified startup/server/window only; Step 2B's first minimal read-only stream smoke failed with local `/stream/:video_id` returning HTTP 500 for sampled search results, so the user-flow e2e remains unverified.
